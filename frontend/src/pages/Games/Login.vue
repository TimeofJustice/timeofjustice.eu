...
<template>
  ...
  <div class="container text-white d-flex flex-column align-items-center justify-content-center">
    ...
    <div class="col-12 col-sm-6 col-md-5 col-lg-4 col-xl-3">
      <BCard class="blur-box border-0" body-class="d-flex flex-column align-items-center gap-2">
        <template #header>
        ...
        <BForm @submit.prevent="submit" class="d-flex flex-column gap-2 w-100">
          <BFormGroup id="input-group-2" label-for="input-2">
            <BFormInput id="input-2" v-model="form.walletId" :placeholder="$t('games.login.enter_wallet')" required :state="validateWalletId" type="password" />
            <BFormInvalidFeedback :state="validateWalletId">
              {{ $t('games.login.error.not_valid') }}
            </BFormInvalidFeedback>
          </BFormGroup>

          <BButton type="submit" variant="primary" class="w-100" :disabled="!validateWalletId">{{ $t('games.login.submit') }}</BButton>
        </BForm>
