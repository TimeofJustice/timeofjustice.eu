...
const cardLoaded = (name: string) => {
  ...
  if (loadedImages.value === gameSession.value.cards.length + gameSession.value.dealerCards.length) {
...
const validateBet = computed(() => {
  return gameSession.value["bet"] >= 10 && gameSession.value["bet"] <= 1000 && gameSession.value["bet"] <= balance;
});
...
const dealDealerCard = () => {
  if (currentShownDealerCard.value < gameSession.value["dealerCards"].length) {
    shownDealerCards.value[currentShownDealerCard.value] = gameSession.value["dealerCards"][currentShownDealerCard.value];
...
const dealPlayerCard = () => {
  if (currentShownCard.value < gameSession.value["cards"].length) {
    shownCards.value[currentShownCard.value] = gameSession.value["cards"][currentShownCard.value];
